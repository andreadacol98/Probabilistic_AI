...
class Densenet(torch.nn.Module):
    ...
    def predict_class_probs(self, x):
        ...
        print("probs_size = {0}".format(probs.size()))
        return probs
...
class BayesianLayer(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        eps = normal_distribution.sample(self.weight_mu.size())
        s_weight = self.weight_mu + torch.exp(self.weight_logsigma) * eps

        if self.use_bias:
            eps = normal_distribution.sample(self.bias_mu.size())
...
class BayesNet(torch.nn.Module):
    ...
    def predict_class_probs(self, x, num_forward_passes=3):
        ...
        for i in range(0, num_forward_passes):
            t = self.forward(x)
            probs += t
        probs = F.softmax(probs/num_forward_passes, dim=1)
