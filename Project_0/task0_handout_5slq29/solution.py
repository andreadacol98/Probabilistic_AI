...
def log_posterior_probs(x):
    ...
    postirior = np.array([0.35, 0.25, 0.4])

    postirior[0] = np.sum(HYPOTHESIS_SPACE[0].logpdf(x)) + np.log(PRIOR_PROBS[0])
    postirior[1] = np.sum(HYPOTHESIS_SPACE[1].logpdf(x)) + np.log(PRIOR_PROBS[1])
    postirior[2] = np.sum(HYPOTHESIS_SPACE[2].logpdf(x)) + np.log(PRIOR_PROBS[2])

    den = logsumexp(postirior)
    log_p = postirior - den

    #postirior = np.exp(postirior - max(postirior))
    #postirior = postirior/sum(postirior)
    #log_p = np.log(postirior)
