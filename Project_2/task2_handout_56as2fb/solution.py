...
class BayesianLayer(torch.nn.Module):
    ...
    def __init__(self, input_dim, output_dim, bias=True):
        ...
        self.weight_logsigma = nn.Parameter(torch.log(.075 * torch.ones(output_dim, input_dim, requires_grad=True)))

        if self.use_bias:
            self.bias_mu = nn.Parameter(torch.zeros(output_dim, requires_grad=True))
            self.bias_logsigma = nn.Parameter(torch.zeros(output_dim, requires_grad=True))
        else:
    ...
    def forward(self, inputs):
        ...
        weight_distribution = torch.distributions.Normal(self.weight_mu, torch.exp(self.weight_logsigma))
        s_weight = weight_distribution.sample()  # sapled weight

        if self.use_bias:
            bias_distribution = torch.distributions.Normal(self.bias_mu, torch.exp(self.bias_logsigma))
            s_bias = bias_distribution.sample()  # sapled biases
        else:
            bias = None

        for i, inp in enumerate(inputs):
            out[i, :] = torch.matmul(s_weight, inp)
            if self.use_bias:
                out[i, :] = out[i, :] + s_bias
        return out
    ...
    def kl_divergence(self):
        ...
        '''
        print("     Leyer KL = {0}".format(kl_loss))
        print(self.weight_mu)
        L2Loss = torch.norm(self.weight_mu-self.prior_mu * torch.ones(self.weight_mu.shape))
        L2Loss += torch.norm(torch.exp(self.weight_logsigma)-self.prior_sigma * torch.ones(self.weight_logsigma.shape))
        print("     L2 norm diff = {0}".format(L2Loss))
        '''
        return kl_loss
...
class BayesNet(torch.nn.Module):
    ...
    def predict_class_probs(self, x, num_forward_passes=10):
        ...
        for i in range(0, num_forward_passes):
            t = self.forward(x)
            probs += t
    ...
    def kl_loss(self):
        ...
        for l in self.layers:
            if type(l) == nn.modules.container.Sequential:
                kl += l[0].kl_divergence()
            else:
                kl += l.kl_divergence()
        return kl
        # TODO: enter your code here // DONE
